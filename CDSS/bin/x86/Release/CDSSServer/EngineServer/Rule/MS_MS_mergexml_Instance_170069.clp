(defrule MS_MS_mergexml_Instance_170069_0
(filepath ?filepath)
(Insolin ?Insolin)
...
(bind ?CIL030 (Leaf equals ?Shuanggua_Drug YES Shuanggua_Drug))
(bind ?CIL031 (Leaf equals ?Gelietong_Drug YES Gelietong_Drug))
(bind ?CIL032 (Leaf equals ?AGI_Drug YES AGI_Drug))
(if
(and (Transform ?CIL030)  (Transform ?CIL031)  (Transform ?CIL032) )
then
(bind ?CIN020 TRUE)
else
(bind ?CIN020 NULL)
...
(if(NotifyOrNot >= ?Threshhold 1 ?ShortData ?filepath MS_mergexml_Instance_170069)
then
(undefrule *)
...
(FactUsed "Insolin" "Shuanggua_Drug" "Gelietong_Drug" "AGI_Drug")
)
)


(defrule MS_MS_mergexml_Instance_170069_1
(filepath ?filepath)
...
(bind ?CIL032 (Leaf equals ?Gelienai_Drug YES Gelienai_Drug))
...
(undefrule *)
(InterpretationIndex "NO_VALUE")
(Recommendation "调整二甲双胍或磺脲类或格列奈类种类或剂量")
(FactUsed "Huangniao_Drug" "Shuanggua_Drug" "Gelienai_Drug" "AGI_Drug")
)
)


(defrule MS_MS_mergexml_Instance_170069_2
(filepath ?filepath)
(Shuanggua_Drug ?Shuanggua_Drug)
(Gelienai_Drug ?Gelienai_Drug)
(Gelietong_Drug ?Gelietong_Drug)
...
(bind ?CIL034 (Leaf equals ?Gelienai_Drug YES Gelienai_Drug))
(bind ?CIL035 (Leaf equals ?Gelietong_Drug YES Gelietong_Drug))
(if
(and (Transform ?CIL034)  (Transform ?CIL035) )
then
(bind ?CIN022 TRUE)
...
(bind ?CIL036 (Leaf equals ?Gelietong_Drug YES Gelietong_Drug))
(bind ?CIL037 (Leaf equals ?AGI_Drug YES AGI_Drug))
(if
...
(if
(or (Transform ?CIN020)  (Transform ?CIN021)  (Transform ?CIN022)  (Transform ?CIN023) )
then
(bind ?CIN010 TRUE)
else
(bind ?CIN010 NULL)
...
(undefrule *)
(InterpretationIndex "NO_VALUE")
(Recommendation "二甲双胍+胰岛素或磺脲类")
(FactUsed "Gelietong_Drug" "Shuanggua_Drug" "Huangniao_Drug" "Gelienai_Drug" "AGI_Drug")
)
)
